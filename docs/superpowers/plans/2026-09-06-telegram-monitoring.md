...
- [ ] Establish the existing monitoring test baseline; retain evaluator semantics.
- [ ] Add a mocked HTTPS transport test, observe failure, implement bounded
  secret-safe delivery, then exercise HTTP/API/network/disabled paths.
- [ ] Add deterministic formatter tests, observe failure, implement reports from
...
- [ ] Add persisted transition tests, observe failure, implement serialized SQLite
...
- [ ] Test and integrate opt-in `--notify`, `--daily-report`, and
...
- [ ] Document environment, writable durable mount, 08:00 Bangkok / 01:00 UTC
  daily cron, proposed periodic monitoring-only cron, disable and smoke test.
- [ ] Independently review standards and specification against baseline
  `1a0c8045d25aeff3fd6ba2c7d12ffa946fa7b4bd`; resolve actionable findings.
- [ ] Run monitoring/notification/backend tests and `corepack pnpm verify`; record
...
- [ ] Commit and push `codex/next-iteration`. Do not deploy, touch VPS, run
